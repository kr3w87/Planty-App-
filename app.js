...
<div class="detail-actions"><button class="primary-action" onclick="water('${id}')">💧 Gegossen</button><button onclick="fertilize('${id}')">🌱 Gedüngt</button><button onclick="openPhoto('${id}')">📸 Foto</button><button onclick="openHealth('${id}')">🩺 Beobachtung</button></div>
...
<div class="detail-tab-panel active" data-detail-panel="overview"><div class="detail-info-grid"><div><small>Licht</small><b>☀️ ${safe(current.light_level||'—')}</b></div><div><small>Gießintervall</small><b>💧 ${current.watering_interval_days||'—'} Tage</b></div><div><small>Düngeintervall</small><b>🌱 ${current.fertilizing_interval_days||'—'} Tage</b></div><div><small>Dokumentiert</small><b>🌿 ${span} Tage</b></div></div><h3>📝 Notizen</h3><div class="health">${safe(current.notes||'Keine Notizen.')}</div></div>
...
/* V3.6 · Seiten-Navigation: vorhandene Funktionen bleiben erhalten, aber werden in klare Bereiche aufgeteilt. */
